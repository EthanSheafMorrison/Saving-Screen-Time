...
import { ZODIAC_SIGNS, rollOOO, type ZodiacId } from './horoscope-data';
...
export default function OOOInline() {
  ...
  const handleGenerate = () => {
    if (!sign) return;
    setOooText(rollOOO(sign.id as ZodiacId, screenTime));
    setPrinted(false);
    setPrinting(true);
  };
...
const CSS = `
  ...
  .ooo-sign-btn:hover, .ooo-st-btn:hover {
    border-color: ${colors.yellow} !important;
    background: rgba(231,255,0,0.06) !important;
  }
  .ooo-sign-btn.ooo-active, .ooo-st-btn.ooo-active {
    border-color: ${colors.yellow} !important;
  ...
  .ooo-inline button:not(.ooo-sign-btn):not(.ooo-st-btn):hover:not(:disabled) {
